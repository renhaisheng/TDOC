...
class Check(object):
        ...
        for smi in finished_smiles:
            ...
            # Deal with runtime errors.
                ...
                # Process the problem of optimization convergence.
                    ...
                    if self.para['gaussian_version'] == 'g09' and max_force < 0.001000:
                        ...
                        with open(smi + '.gjf', 'w') as f:
                            f.write(''.join(lines))
                    elif self.para['gaussian_version'] == 'g16' and max_force < 0.001000:
                        if 'opt=tight' in lines[2]:
                            print('Processed: Decrease convergence accuracy.\n')
                            lines[2] = lines[2].replace('opt=tight', 'opt').replace('opt=(tight,cartesian)', 'opt=cartesian')
                        else:
                            print('Processed: Continue to optimize.\n')
    ...
    def check_MP2_out(self, finished_smiles):
        manually_process = []
        for smi in finished_smiles:
            ...
            # Deal with insufficient memory.
                ...
                added_mem = (atoms.get('C', 0) + atoms.get('O', 0) + atoms.get('N', 0)) * 10
                mem = mem + added_mem
            ...
            # Deal with the problem of SCF convergence.
                ...
                with open(smi + '.gjf') as f:
                    input_content = f.read()
                if 'iptyp' not in input_content:
                    input_content = input_content.replace('hf', 'hf,iptyp=none;maxit,1000')
                    with open(smi + '.gjf', 'w') as f:
                        f.write(input_content)
                    print('Processed: No interpolation.\n')
                elif 'shift' not in input_content:
                    input_content = input_content.replace('maxit,1000', 'maxit,1000;shift,-0.3,0.3')
                    with open(smi + '.gjf', 'w') as f:
                        f.write(input_content)
                    print('Processed: Apply shift level.\n')
                elif 'shift,-0.3,0.3' in input_content:
                    input_content = input_content.replace('shift,-0.3,0.3', 'shift,-0.5,-0.3')
                    with open(smi + '.gjf', 'w') as f:
                        f.write(input_content)
                    print('Processed: Apply shift level.\n')
                elif 'shift,-0.5,-0.3' in input_content:
                    input_content = input_content.replace('shift,-0.5,-0.3', 'shift,-0.8,0')
                    with open(smi + '.gjf', 'w') as f:
                        f.write(input_content)
                    print('Processed: Apply shift level.\n') 
                elif 'shift,-0.8,0' in input_content:
                    input_content = input_content.replace('shift,shift,-0.8,0', 'shift,-1.0,-0.5')
                    with open(smi + '.gjf', 'w') as f:
                        f.write(input_content)
                    print('Processed: Apply shift level.\n')
                else:
        ...
        for smi in finished_smiles:
            ...
            # Deal with the problem of SCF convergence.
                ...
                with open(smi + '.gjf') as f:
                    input_content = f.read()
                if 'iptyp' not in input_content:
                    input_content = input_content.replace('hf', 'hf,iptyp=none;maxit,1000', 1)
                    with open(smi + '.gjf', 'w') as f:
                        f.write(input_content)
                    print('Processed: No interpolation.\n')
                elif 'shift' not in input_content:
