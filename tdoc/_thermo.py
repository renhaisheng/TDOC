...
class Thermo(object):
    ...
    def get_data_from_out(self, smi, method):
        ...
        if Hf_mol:
            if Hf_conf != None:
                ...
                Hf_mol, S, Cp = round(Hf_mol, 3), S, dict(tuple(zip([200, 250, 298.15] + list(range(300, 1000, 50)) + list(range(1000, 5050, 100)), np.round(Cp, 3))))
        return Hf_mol, S, Cp
    ...
    def get_conf_correction_from_GFNFF(self, smi):
        ...
        E, T = E[0] * np.ones(len(E)) - E, 1 / (1.987 * np.array([200, 250, 298.15] + list(range(300, 1000, 50)) + list(range(1000, 5050, 100))))
